...
if direction then
 begin
  ...
  form1.SaveDialog1.title := 'Задайте файл для сохранения шифр-текста';
  ...
  form1.openDialog1.title := 'Выберите файл с шифр-текстом';
 ...
 L1:
 pwd := '';
 ClickedOK := InputQuery(EDDT, 'Введите пароль, от 8 до 16 символов', b);
 if not ClickedOK then exit;
 lb := length(b);
 if (lb < 8) or (lb > 16) then
  begin
   application.MessageBox(pchar('Пароль должен быть длиной от 8 до 16 символов'), appname, mb_ok + mb_iconerror);
   goto L1;
  end;
...
closefile(Fd1);
if direction then
 application.MessageBox(pchar('Шифр-текст успешно сохранен в файле '+FName), appname, mb_ok+MB_ICONASTERISK);
